		...
		<div class="manage">
			<table class="list">
				...
				<tr>
					...
					<td class="w2 c">
					<c:if test="${item.eu_sex==0}">男</c:if>
					<c:if test="${item.eu_sex==1}">女</c:if>
					</td>
					<td>${item.eu_email}</td>
					<td class="w4 c">${item.eu_mobile}</td>
					<td class="w1 c"><a href="EasyByServlet?org=get&id=${item.eu_user_id};">修改</a> <a class="manageDel" href="EasyByServlet?org=del&id=${item.eu_user_id};">删除</a></td>
				</tr>				
				</c:forEach>
			</table>
		</div>
